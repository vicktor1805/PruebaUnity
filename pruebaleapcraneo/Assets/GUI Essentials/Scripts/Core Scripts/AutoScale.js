...
function Scale() {
    if (keepAspectRatio) {
        ...
        if(cambio)
        {
            thisGUITexture.pixelInset.x = posX;
            thisGUITexture.pixelInset.y = posY;
            cambio = false;
        }
        // Update the values for the current screen resolution
        currentScreenWidth = Screen.width;
        currentScreenHeight = Screen.height;        
        //Debug.Log(transform.name + " => X: " + thisGUITexture.pixelInset.x + " Y: " + thisGUITexture.pixelInset.y); 
    }
}
...
function setPixelInsetX(X) {
    posX = X;
    cambio = true;
}

function setPixelInsetY(Y) {
    posY = Y;
    cambio = true;
}